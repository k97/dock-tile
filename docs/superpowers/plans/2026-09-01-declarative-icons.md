...
- Preview fidelity (spec's accepted compromise) intentionally has no task at plan time: the preview was expected to keep rendering the legacy composition as the approximation, revisited only if the matrix in T14 showed it misleading. **Superseded during execution**: Task 6 found the preview geometry actually diverging (full-bleed legacy composition vs. the declarative path's margined tile) well before T14's matrix, and Task 11 fixed it by having the preview adopt `IconDepthMetrics.contentInsetRatio` unconditionally on every macOS version. So the preview did get a task after all — this note is a plan-time assumption that execution overtook, not a description of what shipped.
